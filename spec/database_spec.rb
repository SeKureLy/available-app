...
describe 'Integration Tests of API and Database' do
  ...
  describe 'Retrieve and store project' do
    ...
    it 'HAPPY[paper]: should be able to save paper from Scopus to database' do
      ...
      db_find_by_publication = PaperDeep::Repository::For.klass(PaperDeep::Entity::Paper)
        .find_publication_by_id('84979828304')
      db_find_id = PaperDeep::Repository::For.klass(PaperDeep::Entity::Paper)
        .find_eid('2-s2.0-84979828304')
      ...
      _(db_papers[0].eid).must_equal(papers_parse_result[0].eid)
      _(db_papers[0].title).must_equal(papers_parse_result[0].title)
      _(db_papers[0].paper_link).must_equal(papers_parse_result[0].paper_link)
      _(db_papers[0].citedby_link).must_equal(papers_parse_result[0].citedby_link)
      _(db_papers[0].date).must_equal(papers_parse_result[0].date)
      _(db_papers[0].organization).must_equal(papers_parse_result[0].organization)
      _(db_papers[0].citedby).must_equal(papers_parse_result[0].citedby)
      _(db_papers[0].publication_id).must_equal(papers_parse_result[0].publication_id)
      _(db_papers[0].author).must_equal(papers_parse_result[0].author)
    ...
    it 'HAPPY[publication]: should be able to save paper from Scival to database' do
      ...
      PaperDeep::Repository::For.entity(publications_parse_result.first)
        .db_find_or_create(publications_parse_result.first)
    ...
    it 'HAPPY[paper]+[publication]: should be able to keep consistency from Paper and Publication' do
      ...
      db_publication = PaperDeep::Repository::For.entity(publications_parse_result.first)
        .db_find_or_create(publications_parse_result.first)
